#!/usr/bin/env python3
########################################################################
# Event: 3rd Year CBA Code
# Date 11/3/21
#Author: Eva Colton
# Email: evacolton@stjosephslucan.com
# Purpose: calculate the distance of a hand and turn a servo depending on the distance
########################################################################
...
def pulseIn(pin,level,timeOut):  # obtain pulse time of a pin under timeOut
    ...
    while(GPIO.input(pin) != level):                   #####
        if((time.time() - t0) > timeOut*0.000001):     # the echo pin waits in LOW level until it receives a HIGH level
            return 0;                                  ####
    t0 = time.time()
    while(GPIO.input(pin) == level):
        if((time.time() - t0) > timeOut*0.000001):      ####
            return 0;                                   # when he echo pin recieves a HIGH level it times how long it's high 
...
def getSonar():    # get the measurement results of ultrasonic module,with unit: cm
    ...
    distance = pingTime * 340.0 / 2.0 / 10000.0     # calculate distance with sound of speed 340m/s
    return distance

def setup():
    global p
    GPIO.setmode(GPIO.BOARD)        # use PHYSICAL GPIO Numbering for the breadboard
...
def loop():
    ...
    womp = 0    # create a variable to count how many times the distance is within the limits 
    while(True):
        distance = getSonar()   # get distances
        if 3 < distance < 10:   # set the limits for the distance
